    ...
    public static partial class CoreHelper {
        ...
        public static class ReflectionHelper {
            ...
            public static string GetDescription<T>(string memberName) {
                return GetDescription(typeof(T).GetType(), memberName);
            }
            ...
            /// <param name="typeDescription">类名，包括命名空间,如果类型不处于当前执行程序集中，需要包含程序集名，范例：Test.Core.Test2,Test.Core</param>
            /// <param name="parameters">传递给构造函数的参数</param>        
            public static T CreateInstance<T>(string typeDescription, params object[] parameters) {
                Type type = Type.GetType(typeDescription) ?? Assembly.GetCallingAssembly().GetType(typeDescription);
            ...
            public static T CreateInstance<T>(Type type, params object[] parameters) {
                return CoreHelper.ConvertHelper.To<T>(Activator.CreateInstance(type, parameters));
            }
            ...
            public static List<Assembly> GetAssemblies(string directoryPath) {
                var filePaths = FileHelper.GetAllFiles(directoryPath).Where(t => t.EndsWith(".exe") || t.EndsWith(".dll"));
